...
/* Tuned for bare PCB + solder mask (no LGP); resists sweat false triggers. */
#define TOUCH_THRESHOLD_BASE 42
#define RELEASE_THRESHOLD_BASE 27
...
bool mpr121_init(uint8_t i2c_addr)
{
    ...
    write_reg(i2c_addr, 0x35, 15); // Delay limit Touched (lower: sweat releases faster)
    ...
    write_reg(i2c_addr, 0x5D, 0b01001000); // CT=2us, SFI=4 samples, ESI=1ms
    ...
    /* Autoconfig limits: below stock max (~248) for thin-overlay pads. */
    //     const uint8_t usl = (3.3 - 0.1) / 3.3 * 256;
    const uint8_t usl = 190;
    write_reg(i2c_addr, 0x7D, usl);
    write_reg(i2c_addr, 0x7E, (uint8_t)(usl * 0.65));
    write_reg(i2c_addr, 0x7F, (uint8_t)(usl * 0.85));
